...
class EventLoggerService:
    ...
    def create_and_process_event(self, user_id: str, description: str) -> str:
        """
        ...
        if not self.redis_client.save_redis_object(event):
            raise Exception('Could not save event')
        process_event_for_user.apply_async((event.id, user_id, description))
        return event.id

    def update_event(self, event: EventLoggerRedis) -> EventLoggerRedis:
        """
        ...
        redis_event = self.get_event_by_id(event.id)
        redis_event.status = event.status
        self.redis_client.save_redis_object(redis_event)
        return redis_event

    def get_event_by_id(self, event_id: str) -> EventLoggerRedis:
        """
    ...
    def get_all_events(self, page=0, limit=10) -> tuple[list[EventLoggerRedis], bool, int]:
        """
